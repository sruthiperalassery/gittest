...
public class Tc_Stock_009_StockTypeTransferReconciliationToDamageCreationAndVerificationTestCase extends BaseClassLoader {
	HashMap<String, String> input = new HashMap<String, String>();

	@Test
	public void stockTypeTransferReconciliationToDamageCreationAndVerification() throws Exception {
		input.put("SBU", "MTN");
		...
		input.put("OptionType", "Reconciliation to Damage");
				...
				if (stb.getProductCodeAtRow(i).contains(input.get("ProductCode"))
					...
					if (stb.getNumOfSerialPopupProductRows() >= 1) {
						for (int j = 3; j <= stb.getNumOfSerialPopupProductRows();) {
							if (stb.getSerialPopupCurrentStateAtRow(j).contains("Recons Stock")) {
						...
						for (int i1 = 1; i1 <= stb.getNumOfPopupProductRows();) {
							if (stb.getPopupProductNameAtRow(i1).contains(input.get("ProductName"))
									...
									&& stb.getPopupProductStockQtyAtRow(i1)
											.contains(input.get("ReconsStock") + "/0")) {
		...
		if (stb.getNumOfMainProductRows() >= 1) {
			...
			for (int i = 1; i <= stb.getNumOfMainProductRows();) {
				if (stb.getProductCodeAtRow(i).contains(input.get("ProductCode"))
					...
					Assert.assertEquals(stb.getSalableStockAtRow(i), input.get("ProductSalableStock"));
					...
					Assert.assertEquals(Integer.parseInt(stb.getReconsStockAtRow(i)), Integer.parseInt(input.get("ReconsStock"))+ 1);
					...
					if (stb.getNumOfSerialPopupProductRows() >= 1) {
						for (int j = 3; j <= stb.getNumOfSerialPopupProductRows();) {
							if (Long.parseLong(stb.getSerialPopupFromSerialNoAtRow(j)) <= Long.parseLong(input.get("ProductFromSerialNo")) && Long.parseLong(input.get("ProductFromSerialNo"))<= Long.parseLong(stb.getSerialPopupToSerialNoAtRow(j)) ) {
								Assert.assertEquals(stb.getSerialPopupCurrentStateAtRow(j), input.get("Danaged Stock"));
								break;
							} else {
								if (j <= stb.getNumOfSerialPopupProductRows()) {
									j++;
