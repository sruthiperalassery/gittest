...
public class Tc_Stock_003_ReleaseStockAndVerificationtestCase extends BaseClassLoader {
	HashMap<String, String> input = new HashMap<String, String>();

	@Test
	public void ReleaseStockAndVerification() throws Exception {
		...
		for (int n = 1; n <= 2; n++) {
			...
			if (stockdata.Excelread("ReleaseStockCheck",n,8).equalsIgnoreCase("yes")) {
				...
				input.put("descProductCode", stockdata.Excelread("ReleaseStockCheck",n,0) );
				input.put("descBatchNo",stockdata.Excelread("ReleaseStockCheck",n,2));
				input.put("descMRP",stockdata.Excelread("ReleaseStockCheck",n,3));
				input.put("descProductName",stockdata.Excelread("ReleaseStockCheck",n,1));
				input.put("descProductFromSerialNo",stockdata.Excelread("ReleaseStockCheck",n,5));
				input.put("descProductToSerialNo",stockdata.Excelread("ReleaseStockCheck",n,6));
				input.put("descProductQty",stockdata.Excelread("ReleaseStockCheck",n,7));
			...
			}else if (stockdata.Excelread("ReleaseStockCheck",n,8).equalsIgnoreCase("no"))  {
				
				input.put("nondescProductCode",stockdata.Excelread("ReleaseStockCheck",n,0));
				input.put("nondescBatchNo", stockdata.Excelread("ReleaseStockCheck",n,2));
				input.put("nondescMRP", stockdata.Excelread("ReleaseStockCheck",n,3));
				input.put("nondescProductName", stockdata.Excelread("ReleaseStockCheck",n,1));
				input.put("nondescProductQty",stockdata.Excelread("ReleaseStockCheck",n,7));
		...
		hrs.selectOption("Release Stock");
		...
		if (stockdata.Excelread("ReleaseStockCheck",n,8).equalsIgnoreCase("yes")) {
			if (hrs.getNumOfRows() >= 1) {
				...
				if (stb.getNumOfPopupProductRows() >= 1) {
					for (int i = 1; i <= stb.getNumOfPopupProductRows();) {
						if (stb.getPopupProductNameAtRow(i).contains(input.get("descProductName"))
								...
								&& stb.getPopupProductStockQtyAtRow(i).contains(input.get("descHoldStock") + "/0")) {
				...
				//hrs.clickOnSaveBtn();
				Thread.sleep(1000);
				ast.assertEquals(input.get("PopupProductSalableStock"), input.get("descHoldStock")+"/0", "Popup Product Quantity and Salable stock Quantity is not matching");
			...
			if (hrs.getNumOfRows() >= 1) {
				...
				if (stb.getNumOfPopupProductRows() >= 1) {
					for (int i = 1; i <= stb.getNumOfPopupProductRows();) {
						if (stb.getPopupProductNameAtRow(i).contains(input.get("nondescProductName"))
								...
								&& stb.getPopupProductStockQtyAtRow(i).contains(input.get("nondescHoldStock") + "/0")) {
							input.put("PopupProductCode", stb.getPopupProductCodeAtRow(i));
							input.put("PopupProductName", stb.getPopupProductNameAtRow(i));
							input.put("PopupProductMRP", stb.getPopupProductMRPAtRow(i));
							input.put("PopupProductSalableStock", stb.getPopupProductStockQtyAtRow(i));
				...
				ast.assertEquals(input.get("PopupProductSalableStock"), input.get("nondescHoldStock")+"/0", "Popup Product Quantity and Salable stock Quantity is not matching");
