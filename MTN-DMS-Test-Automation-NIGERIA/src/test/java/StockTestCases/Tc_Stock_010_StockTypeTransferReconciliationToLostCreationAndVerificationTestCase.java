...
public class Tc_Stock_010_StockTypeTransferReconciliationToLostCreationAndVerificationTestCase extends BaseClassLoader {
	HashMap<String, String> input = new HashMap<String, String>();

	@Test
	public void stockTypeTransferReconciliationToLostCreationAndVerification() throws Exception {
		input.put("SBU", "MTN");
		...
		input.put("OptionType", "Reconciliation to Lost");
					...
					if (stb.getNumOfSerialPopupProductRows() >= 1) {
						for (int j = 3; j <= stb.getNumOfSerialPopupProductRows();) {
							if (Long.parseLong(stb.getSerialPopupFromSerialNoAtRow(j)) <= Long.parseLong(input.get("ProductFromSerialNo")) && Long.parseLong(input.get("ProductFromSerialNo"))<= Long.parseLong(stb.getSerialPopupToSerialNoAtRow(j)) ) {
								Assert.assertEquals(stb.getSerialPopupCurrentStateAtRow(j), input.get("In-Transit Lost"));
								break;
							} else {
								if (j <= stb.getNumOfSerialPopupProductRows()) {
									j++;
